...
async function runRealGeminiPipeline(brdText, apiKey) {
  ...
  try {
    ...
    printConsoleLog("[PM Agent] Compiling requirements checklist...", "Product Manager", "var(--color-agent-pm)");
    
    const pmPrompt = `PM Agent: Analyze the following BRD. Output a high-tech terminal-style summary outlining screen targets, core feature checklists, and design parameters.
    BRD: "${brdText}"`;
    ...
    printConsoleLog("[Architect Agent] Formulating theme layout variables...", "System Architect", "var(--color-agent-arch)");

    const archPrompt = `Architect Agent: Design a modern responsive structure and HSL dark theme variables based on these specifications. Summarize the blueprint and color tokens briefly.
    Specs: "${pmAnalysis}"`;
    ...
    printConsoleLog("[Dev Agent] Generating responsive single-page code module...", "Frontend Engineer", "var(--color-agent-fe)");

    const devPrompt = `Developer Agent: Build a beautiful, complete, single-page HTML app. Use vanilla CSS (<style>) and interactive JS (<script>). No markdown wraps. No cut corners. No placeholders.
    PM Specs: "${pmAnalysis}"
    Architect Layout: "${archDesign}"`;
    ...
    printConsoleLog("[QA Agent] Running responsive compliance scanner...", "QA Auditor", "var(--color-agent-qa)");

    const qaPrompt = `QA Agent: Review, audit, and correct this code for any HTML/CSS syntax errors or JS exceptions. Output ONLY the final complete corrected HTML (no markdown):
    Code: "${generatedCode}"`;

    let finalCode = await callGemini(qaPrompt, apiKey, activeModel);
    finalCode = cleanGeneratedCode(finalCode);

    printConsoleLog(`[QA Agent] Visual compliance checklist: PASSED. Approving codebase release!`, "QA Auditor", "var(--color-agent-qa)");
